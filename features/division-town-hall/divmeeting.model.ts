...
import TownhallUtil from './divmeeting.util';
import { InAuthUser } from '@/hooks/interface/in_auth_user';

const EVENT = 'division_meeting';
...
async function findAllEventWithQuery({
  ...
  center: string | null;
}): Promise<InInstantEvent[]> {
  console.log('division:', division);
  console.log('center:', center);
  const divisionRef = FirebaseAdmin.getInstance().Firestore.collection(EVENT).where('division', '==', division);
  const centerRef = FirebaseAdmin.getInstance().Firestore.collection(EVENT).where('center', '==', center);

  const result = await FirebaseAdmin.getInstance().Firestore.runTransaction(async (transaction) => {
    const divisionEventListSnap = division !== undefined ? await transaction.get(divisionRef) : undefined;
    const centerEventListSnap = center !== undefined ? await transaction.get(centerRef) : undefined;
    if (divisionEventListSnap === undefined && centerEventListSnap === undefined) {
      return [];
    }
    const divisionData = divisionEventListSnap !== undefined ? divisionEventListSnap.docs : [];
    const centerData = centerEventListSnap !== undefined ? centerEventListSnap.docs : [];

    const allEvent: InInstantEvent[] = [...divisionData, ...centerData].reduce((acc: InInstantEvent[], doc) => {
...
async function findAllEvent(currentUserUid: string) {
  ...
  if (isOwner) {
    const allEvent = await FirebaseAdmin.getInstance().Firestore.runTransaction(async (transaction) => {
      ...
      const eventListSnap = await transaction.get(eventRef);
      const data = eventListSnap.docs;
      const events: InInstantEvent[] = data.map((doc) => {
        const innerData = doc.data() as InInstantEvent;
  ...
  return findAllEventWithQuery({ division, center });
}
...
async function create({
  ...
  const userInfoRef = FirebaseAdmin.getInstance().Firestore.collection(MEMBER_COLLECTION).doc(createId);
  ...
  await FirebaseAdmin.getInstance().Firestore.runTransaction(async (transaction) => {
    ...
    if (townhallType !== undefined) {
      const userInfo = userInfoDoc.data() as InAuthUser;
      if (townhallType === 'division' && userInfo.division !== undefined && userInfo.division !== null) {
        newInstantEventBody.division = userInfo.division;
      }
      if (townhallType === 'center' && userInfo.center !== undefined && userInfo.center !== null) {
        newInstantEventBody.center = userInfo.center;
      }
    }
...
async function messageListWithUniqueVoter({
  ...
  const result = await FirebaseAdmin.getInstance().Firestore.runTransaction(async (transaction) => {
    ...
    const isShowAll = eventState === 'showAll';
    const isOwnerMember = ownerMemberDoc.exists;
    const isSecret = eventInfo.isSecret !== undefined && eventInfo.isSecret === true;
    // 자신이 속한 division이나 center가 아니면 조회 불가
    if (
      isOwnerMember === false &&
      ((eventInfo.division !== undefined &&
        userInfo.division !== undefined &&
        eventInfo.division !== userInfo.division) ||
        (eventInfo.center !== undefined && userInfo.center !== undefined && eventInfo.center !== userInfo.center))
    ) {
      return {
        list: [],
        uniqueVoterCount: 0,
    ...
    const originData = colDocs.docs.map((mv) => {
      ...
      const returnData = {
        ...docData,
        userName:
          docData.userName && (isOwnerMember || isSecret === false) && isPreview === false
            ? docData.userName
            : undefined,
        email: docData.email && (isOwnerMember || isMyMessage || isSecret === false) ? docData.email : undefined,
        id: mv.id,
        voter: [],
        voted,
                ...
                .map((replyMv) => {
                  if (replyMv.deny !== undefined && replyMv.deny) {
                    return {
                      ...replyMv,
                      userName:
                        replyMv.userName && (isOwnerMember || isSecret === false) && isPreview === false
                          ? replyMv.userName
                          : undefined,
                      email:
...
async function messageInfo({
  ...
  const resp = await FirebaseAdmin.getInstance().Firestore.runTransaction(async (transaction) => {
    ...
    const eventInfo = eventDoc.data() as InInstantEvent;
    const eventState = TownhallUtil.calEventState(eventInfo);
    isSecret = eventInfo.isSecret !== undefined && eventInfo.isSecret === true;
  ...
  return {
    ...resp.docData,
    userName: resp.docData.userName && (isOwnerMember || isSecret === false) ? resp.docData.userName : undefined,
    email:
            ...
            .map((mv) => {
              if (mv.deny !== undefined && mv.deny) {
                return {
                  ...mv,
                  reply: '비공개 처리된 메시지입니다.',
                  userName: mv.userName && (isOwnerMember || isSecret === false) ? mv.userName : undefined,
