...
const CreateTownhallEvent = function ({
  ...
  const [isSecret, setIsSecret] = useState(origin?.isSecret ?? false);
  const [townhallType, setTownhalltype] = useState<'division' | 'center'>('division');
  ...
  return (
    <Box borderWidth="1px" borderRadius="lg" p="2" mt="6" bg="white" display={isShow ? '' : 'none'}>
      ...
      {mode === 'CREATE' && (
        <FormControl mt={4}>
          <FormLabel>부문/센터 전환</FormLabel>
          <Checkbox
            size="md"
            isChecked={townhallType === 'division'}
            onChange={() => {
              setTownhalltype('division');
            }}
          >
            부문 타운홀
          </Checkbox>
          <Checkbox
            marginLeft={2}
            size="md"
            isChecked={townhallType === 'center'}
            onChange={() => {
              setTownhalltype('center');
            }}
          >
            센터 타운홀
          </Checkbox>
