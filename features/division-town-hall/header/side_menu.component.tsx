...
import TownhallClientService from '../divmeeting.client.service';
...
async function immediateCloseSendMessagePeriod({ instantEventId }: { instantEventId: string }) {
  try {
    await TownhallClientService.immediateClosSendMessagePeriod({
      instantEventId,
    });
    return {
      result: true,
...
async function reopenEvent({ instantEventId }: { instantEventId: string }) {
  try {
    await TownhallClientService.reopen({
      instantEventId,
    });
    return {
      result: true,
...
const TownhallHeaderSideMenu = function ({ eventState, instantEventInfo, onCompleteLockOrClose }: Props) {
        ...
        <MenuList>
          ...
          <MenuItem
            onClick={() => {
              ...
              window.open(`/town-hall-meeting/${instantEventInfo.instantEventId}?isPreview=true`, '_blank');
            }}
          >
            프리뷰 모드로 열기
          </MenuItem>
...
export default TownhallHeaderSideMenu;
