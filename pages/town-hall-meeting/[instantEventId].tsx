...
import { Box, Button, Center, Flex, Heading, Select, Spacer, Text, useDisclosure, useToast } from '@chakra-ui/react';
  ...
  const { status, refetch } = useQuery(
    ...
    async () => {
      ...
      const resp = await axios.get<{ list: InInstantEventMessage[]; uniqueVoterCount: number }>(
        `/api/town-hall/messages.list_with_voter_count/${instantEventInfo?.instantEventId}?isPreview=${isPreview}`,
        {
              ...
              <>
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    updateMessage('');
                    setSending(false);
                    setEditorActive(false); // 에디터 비활성화
                  }}
                >
                  등록
                </Button>
